...
async def sympathy_entry(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if arg:
        ...
        if scope is None:
            ...
            return ConversationHandler.END
    elif len(user_tables) == 1:
        scope = user_tables[0][0]
    else:
        tags = [t for t, _ in user_tables]
        logger.info("/sympathy отклонён: неоднозначно, столы=%s", tags)
        await update.message.reply_text(texts.sympathy_ambiguous(tags))
        return ConversationHandler.END
    ...
    if table.stage != Stage.SECOND:
        ...
        await update.message.reply_text(texts.sympathy_closed(scope))
        return ConversationHandler.END

    badge = find_badge_in_table(table, user.id)
    ...
    await update.message.reply_text(texts.sympathy_intro(MAX_SYMPATHIES))
    return ENTER


def _parse_entry(raw: str) -> tuple[int, int] | None:
...
async def on_entry(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if priority < 1:
        logger.warning(
            "Неверный приоритет %d стол=%s badge=%s", priority, data["table"], data["badge"]
        )
        await update.message.reply_text(texts.SYMPATHY_PRIORITY_INVALID)
        return ENTER
    ...
    entries = [(t, p) for (t, p) in data["entries"] if t != target]
    entries.append((target, priority))
...
async def done(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    logger.debug("/done симпатии от %s", _who(update))
    return await _finish(update, context)
