...
START_CLOSED = (
    "<b>SOULMATING / ДВЕРЬ ЗАКРЫТА</b>\n"
    f"{DIVIDER}\n\n"
    "Сейчас анкеты не принимаются. Город остаётся на месте; начало — за организаторами.\n\n"
    "Когда этап откроется, вернись по команде /start."
)
...
def start_open(has_profile: bool) -> str:
    if has_profile:
        return (
            ...
            f"{DIVIDER}\n\n"
            "Она уже в городе. Можно продолжить просмотр или изменить детали."
        )
    return (
        "<b>SOULMATING / СЛУЧАЙ</b>\n"
        "Город велик ровно до той минуты, когда двое выбирают друг друга.\n\n"
        f"{ASK_NAME}"
    )
...
VERIFICATION_APPROVED = (
    "<b>АНКЕТА / В ГОРОДЕ</b>\n"
    f"{DIVIDER}\n\n"
    "Проверка закончена. Теперь можно смотреть тех, с кем встреча не исключена."
)
